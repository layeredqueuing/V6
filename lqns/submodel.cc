 ...
 * $Id: submodel.cc 16909 2024-01-23 20:22:14Z greg $
 *
...
double
MVASubmodel::closedModelUtilization( const Server& station, unsigned int k ) const		// CFS
{
...
void
CFSSubmodel::initializeSubmodel()
{
    MVASubmodel::initializeSubmodel();
    std::for_each( _servers.begin(), _servers.end(), std::mem_fn( &Entity::initGroups ) );
}
...
void
CFSSubmodel::reinitializeSubmodel()
{
    MVASubmodel::reinitializeSubmodel( );
    std::for_each( _servers.begin(), _servers.end(), std::mem_fn( &Entity::initGroups ) );
}
...
CFSSubmodel::build()
{
    ...
    for_each ( _servers.begin(), _servers.end(), std::mem_fn( &Entity::resetGroups ) );
    return *this;
}
...
CFSSubmodel::redistribute::operator()( Entity * server )
{
    ...
    std::for_each( groups.begin(), groups.end(), std::mem_fn( &Group::setSpareStatus ) );
    ...
    if ( !redistributed()
	 ...
	 && std::all_of( groups.begin(), groups.end(), std::mem_fn( &Group::isUtilLessThanShare ) )
	 && std::accumulate( groups.begin(), groups.end(), 0., Group::sum( &Group::utilization ) ) <= processor->copies() * 0.99 ) {
	...
	if ( available_share > 0. &&  denominator > 0. ) {
		...
		for_each ( tasks.begin(), tasks.end(), std::mem_fn( &Task::reset_lowerbound ) );
    ...
    for_each ( groups.begin(), groups.end(), std::mem_fn( &Group::computeCFSDelay ) );
}
