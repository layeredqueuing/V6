 ...
 * $HeadURL: http://rads-svn.sce.carleton.ca:8080/svn/lqn/trunk-V6/lqns/activity.cc $
 *
 ...
 *
 * ------------------------------------------------------------------------
 * $Id: activity.cc 17192 2024-04-30 21:20:30Z greg $
 * ------------------------------------------------------------------------
 */
...
void 
Activity::setMaxCustomers(Entry * anEntry, const Activity::Collect& ) const
{
    const_cast<Activity *>(this)->_maxCusts = anEntry->getMaxCustomers();
    if ( flags.trace_customers ) {
	std::cout<<"Activity ("<<name()<<" ) set maxCustomers to "<< _maxCusts<<std::endl;
    }
}

/* ------------------------- Printing Functions ------------------------- */
