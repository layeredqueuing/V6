/*  -*- c++ -*-
 * $HeadURL: http://rads-svn.sce.carleton.ca:8080/svn/lqn/trunk/lqns/unit-test/testmixed.cc $
 *
 * Example from:
 *
 * ------------------------------------------------------------------------
 * $Id: testmixed.cc 13414 2018-10-23 16:16:04Z greg $
 * ------------------------------------------------------------------------
 */
...
run( const unsigned solver_set, const unsigned special )
{
    ...
    for ( unsigned i = 0; i <= EXPERIMENTAL_SOLVER; ++i ) {
	Population N(0);
      ...
      const unsigned special )
{
	...
	break;
    case EXPERIMENTAL_SOLVER:
	closedModel = new Linearizer3( Q, N, Z, priority );
	break;
    }
