/* -*- c++ -*-
 * $Id: mva.cc 14000 2020-10-25 12:50:53Z greg $
 *
...
MVA::step( const Population& N, const unsigned currPri )
{
...
#if defined (DEBUG2)
	    cout << "Sum of residence time for a client chain:  "<<sum<< endl;
#endif
	}
	...
	for ( m = 1; m <= M; ++m ) {
		...
		if ( !isfinite( X[n][k] ) || X[n][k] == 0.0 ) {			/* inf */
		    ...
		    L[n][m][e][k] =   X[n][k]  * Q[m]->R(e,k);	/* Eq:6 */
...
/* Rate of queueing delay caused by non interlocked flow */
void
MVA::setNonILRate(const Population &N ) const
{
    for ( unsigned m = 1; m <= M; ++m ) {
	for ( unsigned k = 1; k <= K; ++k ) {
	    Q[m]->setNonILRate(  *this, k , N);
	}
    }
}

void
MVA::setNonILRate( ) const
{
    for ( unsigned m = 1; m <= M; ++m ) {
	const unsigned E   = Q[m]->nEntries();
	for ( unsigned k = 1; k <= K; ++k ) {
	    for ( unsigned e = 1; e <= E; ++e ) {
		Q[m]->setNonILRate(  *this,  k , e);
	    }
	}
    }
}
...
double
MVA::sumOf_NOIL_m( const Server& station, const Population &N, const unsigned j ) const
{
    assert( 0 < j && j <= K );
    ...
    return (sum>0.05)?sum:0.0;
}

/*
 * Return number of customers from any non interlocked flow
 * at queue `m' by entries.
 */
...
MVA::sumOf_L_m( const Server& station, const Population &N, const unsigned j ) const
{
	...
	for ( unsigned e = 1; e <= E; ++e ) {
	    sum += L[Nej][m][e][k] ;
#if	defined(DEBUG2)
	    if (L[Nej][m][e][k]>0 || LL[Nej][m][e][k]>0)
		cout << "sumOf_L_m():j="<<j<<",L(Nej=" <<Nej<<",m= "<<m<<",e= "
		     <<e<<",k="<<k<<")="<<L[Nej][m][e][k]<<endl;
#endif
	}
    }
    return sum;
...
/*
 * Return number of customers regardless of class at queue `m'.
 * Subtract 1 customer from class `j' by entries.
 * L[Nej][m][e][k] is unmodified queue length with one customer less!
 *
 * because Linearizer2 do not have this the operation.
 * ignores Adjustment for `tau' done .
 */

double
MVA::sumOf_L_m( const Server& station, const Population &N, const unsigned je, const unsigned j ) const
{
    assert( 0 < j && j <= K );
...
double
MVA::sumOf_QL_m( const Server& station, const Population &N, const unsigned j ) const
{
    assert( 0 < j && j <= K );
...
/*
 * Return number of customers regardless of class at queue `m' multiplied
 * by the service time for the class j at the entry je.  Subtract 1 customer
 * from class `j'.
 *
 * L[Nej][m][e][k] is unmodified queue length with one customer less!
 */
...
double
MVA::sumOf_SL_m( const Server& station, const Population &N, const unsigned je, const unsigned j ) const
{
    assert( 0 < j && j <= K );
...
double MVA::avg_S_m(const Server& station, const Population &N, const unsigned k, const unsigned je ) const
{
    assert( 0 < k && k <= K );
    ...
    return (sum_l>0.)? sum/sum_l:0.0;
}


double
MVA::sumOf_SQL_m( const Server& station, const Population &N, const unsigned j ) const
{
    assert( 0 < j && j <= K );
...
double
MVA::queueOnly_adjusted( const Server& station, const unsigned k, const Population& N, const unsigned j ) const
{
    assert( 0 < k && k <= K && 0 < j && j <= K );
	...
	if (station.chainILRate(j)==0.0){
	    ...
	    sum += max( L[Nej][m][e][k] * station.interlock_rate( e, k )- U[Nej][m][e][k], 0.);
	}
    }
    return sum;
}
...
MVA::queueOnly( const Server& station, const unsigned k, const Population& N, const unsigned j ) const
{
	...
	const double service = station.S(e,k);
	if ( service == 0.0 ) continue;
	sum += max( L[Nej][m][e][k] - U[Nej][m][e][k], 0.);
    }
    return sum;
}
...
MVA::print( ostream& output ) const
{
	...
	for ( unsigned e = 1; e <= E; ++e ) {
		...
		output << ", Q_" << e << k << " = " << setw(width);
		if ( !isinf( Q[m]->W[e][k][0] ) ) {
		    output << max(Q[m]->W[e][k][0] - Q[m]->S(e,k), 0.);
		} else {
		    output << Q[m]->W[e][k][0];
		}
...
ostream&
MVA::printR( ostream& output ) const
{
    for ( unsigned m = 1; m <= M; ++m ) {
	const unsigned E = Q[m]->nEntries();
	for ( unsigned e = 0; e <= E; ++e ) {
	    for ( unsigned k = 1; k <= K; ++k ) {
		if ( k > 1 ) output << ", ";
		output << "RCust_{" << m << "," << e << "," << k << "} = " << Q[m]->getRealCustomer(e,k);
	    }
	    output << endl;
	}
    }
...
SchweitzerCommon::estimate_Lm( const unsigned m, const Population & N, const unsigned n ) const
{
	...
	for ( unsigned k = 1; k <= K; ++k ) {
	    if ( N[k] < 1 ) continue;
	    const double scaling = tau_overlap( *Q[m], j, k, N );			/* BUG 145 */

	    for ( unsigned e = 1; e <= E; ++e ) {

		const double N_k = min(static_cast<double>(Q[m]->getMaxCustomers(e,k)), static_cast<double>(N[k]));
		/* this entry has zero or one customer, L(Nej
		 */
		if ( N_k < 1 ) continue;
		...
		/* adjusting the L[Nej][m][e][k] */
		L[Nej][m][e][k] = L_ej * scaling;

		/* sending interlock parent*/
		if ( k == j && Q[m]->IL_Relation(e, k, 1, 0 ) == 3 && N_k > 1 ){
		    if ( N_k < 4 ) {		// L(nej) = L(Nj)* (N-1)/(N+1) when N_k=2;
			L[Nej][m][e][k] *= N_k / (N_k+1.0);
		    } else {
			L[Nej][m][e][k] *= N_k / (N_k-1.0) * 0.75;
		    }
		}

		/* Now estimate utilizations based on queue lengths. */
		if ( L_n_m_e_k > 0.0 ) {
		    U[Nej][m][e][k] =L[Nej][m][e][k]/ L_n_m_e_k * U[n][m][e][k];
		} else {
		    U[Nej][m][e][k] = 0.0;
		}
	    }
...
/*
 * Because of interlocking problems, some core() have to be skipped;
 * particularly, chain c has non interlocked flow only,
 * AND the rest chains are all interlocked flows.
 * for example, model4, if N= {1,1,0}, core(N) must be skipped.
 * see interlocking paper.
 */
...
Linearizer::solve()
{
    ...
    /* If the submodel has interlocked server station,
     * then this mva submodel is solved by SchweizterMVA only.
