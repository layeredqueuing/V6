 ...
 *
 * $Id: help.cc 14037 2020-11-04 19:39:56Z greg $
 */
...
ostream&
Help::flagJSON( ostream& output, bool verbose ) const
{
    output << "Generate JSON output regardless of input format." << endl;
    return output;
}
...
ostream&
Help::traceCFS( ostream & output, bool verbose ) const
{
    output << "Print out CFS computation before each submodel is solved." << endl;
    return output;
}
...
ostream&
Help::traceCustomers( ostream & output, bool verbose ) const
{
    output << "Print the real number of customers or maximum customers traces." << endl;
    return output;
}
...
ostream&
Help::pragmaMVAFractional( ostream& output, bool verbose ) const
{
    output << "Perform Fractional Bard-Schweitzer approximate MVA, which allows fractional customer number. "<<endl;
	return output;
}
...
HelpTroff::preamble( ostream& output ) const
{
    ...
    output << __comment << " $Id: help.cc 14037 2020-11-04 19:39:56Z greg $" << endl
	   << __comment << endl
	   << __comment << " --------------------------------" << endl;
...
HelpLaTeX::preamble( ostream& output ) const
{
    ...
    output << __comment << "  -*- mode: latex; mode: outline-minor; fill-column: 108 -*- " << endl
	   ...
	   << __comment << "" << endl
	   << __comment << " $HeadURL: http://rads-svn.sce.carleton.ca:8080/svn/lqn/trunk/lqns/help.cc $" << endl
	   << __comment << " Original Author:     Greg Franks <greg@sce.carleton.ca>" << endl
	   << __comment << " Created:             " << date << endl
	   << __comment << "" << endl
	   << __comment << " ----------------------------------------------------------------------" << endl
	   << __comment << " $Id: help.cc 14037 2020-11-04 19:39:56Z greg $" << endl
	   << __comment << " ----------------------------------------------------------------------" << endl << endl;
