 ...
 *
 * ------------------------------------------------------------------------
 * $Id: task.cc 14144 2020-11-26 19:37:10Z greg $
 * ------------------------------------------------------------------------
 */
...
Task::modifyParentClientServiceTime( const MVASubmodel& submodel, const Entity * server ) const
{
	...
	if ( flags.trace_interlock ) {
	    std::cout <<"In submodel "<<submodel.number()<<",modifyParentClientServiceTime(server= "<<server->name()<<", client="<<name()<<"), common entry ("<<(*entry)->name()<<"):";
	}
