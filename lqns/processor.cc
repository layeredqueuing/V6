 ...
 *
 * ------------------------------------------------------------------------
 * $Id: processor.cc 14096 2020-11-15 13:58:05Z greg $
 * ------------------------------------------------------------------------
 */
...
/*
 *  initial groups and tasks within a group;
 *  this ensure lqx initialize groups properly.
 */
...
/*
 * Run DPS iff... processor utilization > 1 -- need more delay.
 *                          utilization < 1 && delay for group > 0
 */

bool
CFS_Processor::runDPS() const
{
    if ( utilization() / copies() > 1.02 ) return true;
    return false;
}
...
CFS_Processor::saveServerResults( const MVASubmodel& submodel, double relax )
{
	...
	for ( unsigned ix = 1; ix <= chain.size(); ++ix ) {
	    const unsigned k = chain[ix];
	    if ( hasServerChain(k) ) {
		utilization += submodel.closedModel->utilization( *station, k );
	    }
	}
	...
	if ( flags.trace_cfs ) {
	    ...
	    if ( utilization * group->size() > group->getShare() ) {
		std::cout << " *over*";
	    }
	    std::cout << std::endl;
	}
