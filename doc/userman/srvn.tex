...
%%
%% ------------------------------------------------------------------------
%%  $Id: srvn.tex 14499 2021-02-27 23:24:12Z greg $
%% ------------------------------------------------------------------------
%%
...
The \nonterminal{ternary-expression} may be any valid ternaray expression as discussed above.  Note that in this section,
the report indication variables\index{variables!report indication} and the parameter
variables\index{variables!parameters} may both be used together. 
...
If the \texttt{plot()}\index{Spex!report!plot} function is used, output will formatted in such a way that it
can be used as input into gnuplot\index{gnuplot}.  The first argument to the \texttt{plot}\index{plot}
function call is the x\index{plot!x} variable for the graph.  Multiple additional arguments are permitted
