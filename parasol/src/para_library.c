/* $Id: para_library.c 17458 2024-11-12 11:54:17Z greg $ */
...
ps_task_t * volatile  ps_htp;			/* HOT task pointer	*/
volatile double	ps_now;				/* current time		*/
ps_table_t	ps_task_tab;			/* task table		*/
long		ts_flag;			/* trace state flag	*/
volatile double	ps_run_time;			/* stop time		*/
...
#if	!HAVE_SIGALTSTACK && (!HAVE_UCONTEXT_H || defined(__OSX_AVAILABLE_BUT_DEPRECATED) || defined(__POWERPC__)) || _WIN32 || _WIN64
...
/*	Have to cook task names if angio tracing is on.			*/
	...
	tp->hp = NULL_HOST_PTR;
	if ( !(tp->stack_base = malloc(stacksize)) ) {
	    ps_abort("Insufficient memory");
	}
#if	HAVE_MAKECONTEXT && !defined(__OSX_AVAILABLE_BUT_DEPRECATED) && !defined(__POWERPC__)
	if ( getcontext( &tp->context.u ) < 0 ) {
	    return OTHER_ERR("getcontext fails.");
	}
	tp->context.u.uc_stack.ss_sp = tp->stack_base;
	tp->context.u.uc_stack.ss_size = stacksize;
#endif
	if(!tp->stack_base) {
		ps_abort("Insufficient memory");
	}
...
#if	HAVE_MAKECONTEXT && !defined(__OSX_AVAILABLE_BUT_DEPRECATED) && !defined(__POWERPC__)
	makecontext( &tp->context.u, tp->code, 0 );
#elif	HAVE_SIGALTSTACK && !_WIN32 && !_WIN64
...
)
{
	...
	if(ts_flag) {
	        snprintf(string, TEMP_STR_SIZE, "migrating to node %ld", node);
		ts_report(tp, string);
	}
...
{
	ps_port_t	*pp;			/* port pointer		*/
	long	port = get_table_entry(&ps_port_tab);	/* port set port	*/
...
)
{
	...
	long	dispatcher = ps_create(string, ps_htp->node, ps_htp->host, 
				       shared_port_dispatcher, MAX_PRIORITY);			/* dispatcher task id	*/
	snprintf(string, TEMP_STR_SIZE, "%s - Shared Port Dispatcher", ps_htp->name);
	if(ps_resume(dispatcher) == SYSERR)
...
)
{
	...
	snprintf(string,TEMP_STR_SIZE,"%s Surrogate", ps_htp->name);
...
)
{
	...
	if(ts_flag) {
	        snprintf(string, TEMP_STR_SIZE, "resetting semaphore %ld to value %ld", sid, value);
		ts_report(ps_htp, string);
	} 
	if(angio_flag) {
...
)
{
	...
	np = node_ptr(nid);
 	...
 	for (i = 0; i < ncpu; i++) {
	        snprintf(string, TEMP_STR_SIZE, "%s Scheduler (cpu %ld)", name, i);
		...
		snprintf (string, TEMP_STR_SIZE, "%s Catcher (cpu %ld)", name, i);
...
)
{
	...
	if (SF_PER_TASK_NODE) {
		if (!(gp->ts_tab = (ps_table_t*)malloc(sizeof(ps_table_t))))
			ps_abort("Insufficient Memory");
		init_table2(gp->ts_tab, DEFAULT_MAX_TASKS, sizeof(long));
		for (j = 0; j < DEFAULT_MAX_TASKS; j++)
...
LOCAL 	void 	shared_port_dispatcher()
...

{
	...
	long queue_port = ps_allocate_port("Message Queue", ps_myself);
	if(queue_port == SYSERR)
		ps_kill(ps_myself);
	state = 0;
	while(TRUE) {
...
)
{
	...
	if(event_fl == NULL_EVENT_PTR) {
	        unsigned int i;	    
	        event_fl = (ps_event_t *) malloc(100*sizeof(ps_event_t));
		ep = event_fl;
		if(!ep)
			ps_abort("Insufficient memory");
		for( i = 1; i < 100; i++) {
...
{
	ps_node_t *np = node_ptr(tp->node);	/* node pointer		*/
	long	host = NULL_HOST;		/* host cpu id index	*/
	long	i;				/* loop index		*/
...
)
{
...
#if HAVE_MCHECK
	if ( mprobe( tabp ) != MCHECK_OK ) abort();
#endif

	if(tabp->used == tabp->tab_size) {
		long nbytes = tabp->tab_size * tabp->entry_size;	/* byte count		*/
		long new_size = 2 * tabp->tab_size;			/* new table size	*/
		void * new_tab = malloc(2*nbytes+sizeof(double));	/* new table pointer	*/
		memset(new_tab, 0, 2*nbytes);
		if(!new_tab) ps_abort("Insufficient memory");
		memcpy( new_tab, tabp->tab, nbytes );
		free(tabp->tab);
		tabp->tab = new_tab;
	...
	id = tabp->rover;
	for( i = 0; i < tabp->tab_size; i++) {
		if(!*((long *)(tabp->tab+id*tabp->entry_size)))
			break;
		id = (id + 1) % tabp->tab_size;
	}
//	memset(tabp->tab+id*tabp->entry_size, '\0', tabp->entry_size);
	...
	char string[TEMP_STR_SIZE];  /* is this long enough for any string encounterable? */
		...
		if ((headroom = (ps_headroom() - ps_trsct)) < 
		    10*sizeof(double)*sp_delta) {  /* what value to use here ? */
		        snprintf(string, TEMP_STR_SIZE, "ps_headroom reports %ld for task %ld", 
			    headroom, ps_myself);
			warning(string);
		}
#endif
...
)
{
	...
	np = node_ptr(tp->node);
	if(ts_flag) {
		snprintf(string, TEMP_STR_SIZE, "priority adjusted to %ld", priority);
		ts_report(tp, string);
	}
	switch (tp->state) {
...
{
	char	string[TEMP_STR_SIZE];		/* warning message	*/

	if(w_flag && (ps_htp->priority <= MAX_PRIORITY)) {
	  	snprintf(string, TEMP_STR_SIZE, "%s: Invalid '%s' parameter.", function, param);
	  	warning(string);
	}
	return(SYSERR);
}
...
)
{
	...
	if(w_flag && (ps_htp->priority <= MAX_PRIORITY)) {
		snprintf(string, TEMP_STR_SIZE, "%s: %s.", function, message);
		warning(string);
	}
	return(SYSERR);
}
...
)
{
	...
	if(w_flag && (ps_htp->priority <= MAX_PRIORITY)) {
		snprintf(string, TEMP_STR_SIZE, "%s: An error occured while %s.", function,
		    message);
		warning(string);
	}
	return(SYSERR);
...
)
{
	...
	if(!pp->nmess && time_out < 0.0) {
		*typep = ACK_TIMEOUT;
		if(ts_flag) {
			snprintf(string, TEMP_STR_SIZE, "times out on port %ld", port);
			ts_report(ps_htp, string);
		}
		return(FALSE);
	}
	...
	if(!pp->nmess) { 	/* begin waiting for message		*/	
		if(ts_flag) {
			snprintf(string, TEMP_STR_SIZE, "receiving (blocked) on port %ld", 
		 	   port);
			ts_report(ps_htp, string);
		}
	...
	*mid = mp->mid;
	...
	if(ts_flag) {
		if(mp->type == ACK_TIMEOUT)
			...
			snprintf(string, TEMP_STR_SIZE, "receives message %ld on shared port %ld",
			    *mid, *opp); 
		else if (pp->state == PORT_SET)
			snprintf(string, TEMP_STR_SIZE, "receives message %ld on port %ld", 
			    *mid, *opp);
		else
			snprintf(string, TEMP_STR_SIZE, "receives message %ld on port %ld", 
				*mid, port);
...
)
{
	...
	if(ts_flag && ps_htp != DRIVER_PTR) {
		if (pp->state == PORT_SHARED) {
			snprintf(string, TEMP_STR_SIZE, "sending message %ld to shared port %ld",
			    mp->mid, port);
			ts_report(ps_htp, string);
		}
		else if (ps_task_ptr(pp->owner)->code == port_set_surrogate) {
			snprintf(string, TEMP_STR_SIZE, "sending message %ld to task %ld via port %ld",
...
)
{
	...
	while(lock != NULL_LOCK) {
		lp = lock_ptr(lock);
		if(ts_flag) {
			snprintf(string, TEMP_STR_SIZE, "unlocking lock %ld", lock);
			ts_report(ps_htp, string);
		}
		if(lp->queue == NULL_TASK) {
			lp->owner = NULL_TASK;
			...
			ctp->tep = add_event(ps_now, END_BLOCK, 
			    (long *)lp->owner);
			if(ts_flag) {
				snprintf(string, TEMP_STR_SIZE, "locking lock %ld", lock);
...
/* WCS - 6 May 1998 - Added test for port_set_surrogate.  Others ??? */
			if (tp->code != port_set_surrogate) {
				...
				if (*stat == -1) {
					snprintf(string, TEMP_STR_SIZE, "%s (cpu %ld) task %ld Utilization", 
...
#if !HAVE_SIGALTSTACK && (!HAVE_UCONTEXT_H || defined(__OSX_AVAILABLE_BUT_DEPRECATED) || defined(__POWERPC__)) || _WIN32 || _WIN46
...

{
	...
	fprintf ( stderr, "Testing stacks, this may take a while...\n");

	for (task = 0; task < ps_task_tab.tab_size; task++)
		if (ps_task_ptr(task)->state != TASK_FREE)
			test_stack(task);

	fprintf (stderr, "sp_delta = %ld\n", sp_delta);
 	fprintf (stderr, "sp_dss = %ld\n", sp_dss);
