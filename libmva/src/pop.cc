/*  -*- c++ -*-
 * $Id: pop.cc 15396 2022-01-27 02:37:32Z greg $
 *
...
unsigned 
FullPopulationMap::offset_e_j_mc( const Population & N, const unsigned j,const unsigned mc ) const
{
    unsigned i = offset( N );

    assert( N[j] > 0 && i >= _stride[j]);

    return i - (N[j]-mc)*_stride[j];
}

...
unsigned 
PartialPopulationMap::offset_e_j_mc( const Population & N, const unsigned j,const unsigned mc ) const
{
    unsigned i = offset( N );

    assert( N[j] > 0 && i >= _stride[j]);

    return i - mc*_stride[j];
}
...
unsigned 
SinglePopulationMap::offset_e_j_mc( const Population & N, const unsigned j, const unsigned mc ) const
{
    assert( 0 );	/* Temp -- need to figure this one out. */
    unsigned i = offset( N );
