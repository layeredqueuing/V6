/* -*- C++ -*-
 * $Id: multserv.cc 16975 2024-01-29 20:37:42Z greg $
 *
...
Positive
Reiser_Multi_Server::sumOf_SQL( const MVA& solver, const Population& N, const unsigned k ) const
{
    return solver.sumOf_SQL_m( *this, N, k ) + S( solver, N ) * solver.sumOf_P( *this, N, k );
}
...
Positive
Phased_Reiser_Multi_Server::sumOf_SQL( const MVA& solver, const Population& N, const unsigned k ) const
{
    return Reiser_Multi_Server::sumOf_SQL( solver, N, k ) + solver.sumOf_S2U_m( *this, N, k );
}

...
Positive
Reiser_PS_Multi_Server::sumOf_QL( const MVA& solver, const Population& N, const unsigned k ) const
{
    return solver.sumOf_QL_m( *this, N, k ) + solver.sumOf_P( *this, N, k );
}

/*----------------------------------------------------------------------*/
/*                         Conway Multi Server.                         */
...
double
Conway_Multi_Server::effectiveBacklog_adjusted( const MVA& solver, const Population& N, const unsigned k ) const
{
    Positive sum = 0.0;
	...
	sum += sumOf_PS_k( solver, N, k, step ) * solver.queueOnly_adjusted( *this, i, N, k );
    }
    return sum;
}
...
Markov_Phased_Conway_Multi_Server::wait( const MVA& solver, const unsigned k, const Population& N ) const
{
	...
	for ( unsigned p = 0; p <= MAX_PHASES; ++p ) {
	    ...
	    //  if ( W[e][k][p] >0)
	    //		      cout<<"server[m="<<closedIndex<<"]:w[e="<<e<<",k="<<k<<",p="<<p<<"]= "<<W[e][k][p] <<",s="<<S(e,k,1)<<",sum="<<sum<<",minovertaking= "<< W[e][k][p] - S(e,k,1) - sum<<endl;
...
Markov_Phased_Conway_Multi_Server::meanMinimumOvertaking( const MVA& solver, const Population& N, const unsigned k, const unsigned p_i ) const
{
    ...
    const double n_client = std::min( static_cast<double>(n_server), getMaxCustomers() );
    const Probability prob = PrOT( k, p_i );

    const double mean_cust = static_cast<double>(n_server)/static_cast<double>(n_client);
    /*cout <<"n_server ="<<n_server<<", n_client = "<<n_client
      <<" mean_cust ="<< mean_cust<<endl;*/
    if (mean_cust >2) return 0.0;
...
{
    return PBusy( solver, N, k ) * solver.sumOf_L_m( *this, N, k ) / mu();
}

/* ------------------------ Rolia Multi Server  ----------------------- */
