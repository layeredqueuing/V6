/* -*- c++ -*-
 * $Id: ph2serv.cc 15662 2022-06-09 23:45:43Z greg $
 *
...
Markov_Phased_Server::overtaking( const unsigned k, const unsigned p_i ) const
{
	...
	for ( unsigned p_j = 2; p_j <= P; ++p_j ) {
	    ...
	    //cout<<"	prOvertake[e="<<e<<"][k="<<k<<"][p_i="<<p_i<<"][p_j="<<p_j<<"]="<<prOvertake[e][k][p_i][p_j] <<" residual( e, k, p_j )="<< residual( e, k, p_j )<<endl;
	}
    }
    return sum;
}
...
/*
 * Return the overtaking probability of ph2 or ph3 of a server entry
 */

Probability
Markov_Phased_Server::PrOT_e( const unsigned e, const unsigned p_j ) const
{
    //Probability sum = 0.0;
    double sum= 0.0;
    for ( unsigned k = 1; k <= K; ++k ) {

	//cout<<"PrOT_e(): prOvertake[e="<<e<<"][k="<<k<<"][0][p_j="<<p_j<<"]="<<prOvertake[e][k][0][p_j]<<endl;
	sum += prOvertake[e][k][0][p_j];// not prOvertake[e][k][0][p_j];
    }

    return (sum>1.0)? Probability(1):Probability(sum);
}
...
HVFCFS_Markov_Phased_Server::wait( const MVA& solver, const unsigned k, const Population& N ) const
{
    ...
    double sum = solver.sumOf_SQ_m( *this, N, k ) + solver.sumOf_rU_m( *this, N, k );
    if ( sum < 0.0 ) sum = 0.0;
    //cout<<" solver.sumOf_SQ_m= "<< solver.sumOf_SQ_m( *this, N, k ) <<" , solver.sumOf_rU_m= "<< solver.sumOf_rU_m( *this, N, k )<<", sum="<< sum <<endl;
