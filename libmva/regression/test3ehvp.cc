/* -*- c++ -*-
 * $HeadURL: http://rads-svn.sce.carleton.ca:8080/svn/lqn/branches/merge-V5-V6/libmva/regression/test3ehvp.cc $
 *
 * Pg 264, Lazowska test.
 * FCFS with class dependent average service times.  Use HV server rather
 * than straight FCFS server.  Use HV server with phases, but with only
 * one phase (mainly tests inheritance)
 *
 *
 * ------------------------------------------------------------------------
 * $Id: test3ehvp.cc 15385 2022-01-25 11:45:59Z greg $
 * ------------------------------------------------------------------------
  */
...
test( Population& NCust, Vector<Server *>& Q, VectorMath<double>& Z, VectorMath<unsigned>& priority, const unsigned s5k_ix )
{
    ...
    Q[5] = new HVFCFS_Rolia_Phased_Server(4,classes,2);	/* CPU */
