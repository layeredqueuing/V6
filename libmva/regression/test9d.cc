/*  -*- c++ -*-
 * $HeadURL: http://rads-svn.sce.carleton.ca:8080/svn/lqn/branches/merge-V5-V6/libmva/regression/test9d.cc $
 *
 * Multiserver test.  Conway / de Souza e Silva and Muntz waiting time.
 *
 * ------------------------------------------------------------------------
 * $Id: test9d.cc 15385 2022-01-25 11:45:59Z greg $
 * ------------------------------------------------------------------------
 */
...
test( Population& NCust, Vector<Server *>& Q, VectorMath<double>& Z, VectorMath<unsigned>& priority, const unsigned s22_ix )
{
    ...
    default:
	std::cerr << "Invalid S22 index (0-4):" << s22_ix << std::endl;
	exit( 1 );
    }
}
...
check( const int solverId, const MVA & solver, const unsigned s22_ix )
{
	...
	for ( unsigned k = 1; k <= solver.K; ++k ) {
		...
		std::cerr << ".  Computed=" << solver.L[n][m][1][k] << ", Correct= " << goodL[s22_ix][solverId][m][k] << std::endl;
		ok = false;
	    }
	}
    }
