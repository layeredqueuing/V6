/*  -*- c++ -*-
 * $HeadURL: http://rads-svn.sce.carleton.ca:8080/svn/lqn/branches/merge-V5-V6/libmva/regression/test3.cc $
 *
 * Pg 264, Lazowska test.
 * FCFS with class dependent average service times. 
 *
 * ------------------------------------------------------------------------
 * $Id: test3.cc 15385 2022-01-25 11:45:59Z greg $
 * ------------------------------------------------------------------------
 */
...
void special_check( std::ostream& output, const MVA& solver, const unsigned )
{
    const unsigned m = 6;
    const unsigned k = 1;
...
check( const int solverId, const MVA & solver, const unsigned s5k_ix )
{
	...
	for ( unsigned k = 1; k <= solver.K; ++k ) {
		...
		std::cerr << ".  Computed=" << solver.L[n][m][1][k] << ", Correct= " << goodL[s5k_ix][solverId][m][k] << std::endl;
		ok = false;
	    }
	}
    }
