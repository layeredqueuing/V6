/*  -*- c++ -*-
 * $HeadURL: http://rads-svn.sce.carleton.ca:8080/svn/lqn/branches/merge-V5-V6/libmva/regression/test1hv.cc $
 *
 * Test case from:
 ...
 * ------------------------------------------------------------------------
 * $Id: test1hv.cc 15385 2022-01-25 11:45:59Z greg $
 * ------------------------------------------------------------------------
 */
...
test( Population& NCust, Vector<Server *>& Q, VectorMath<double>& Z, VectorMath<unsigned>& priority, const unsigned )
{
    ...
    Q[1] = new Infinite_Server(2);
    Q[2] = new HVFCFS_Server(2);

    Q[1]->setService(1,8.0).setVisits(1,1.0);
    Q[1]->setService(2,0.0).setVisits(2,0.0);
    Q[2]->setService(1,1.0).setVisits(1,1.0).setVariance(1,1,1,1.0);
    Q[2]->setService(2,1.0).setVisits(2,1.0).setVariance(1,2,1,1.0);
}
