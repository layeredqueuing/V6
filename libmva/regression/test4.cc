/*  -*- c++ -*-
 * $HeadURL: http://rads-svn.sce.carleton.ca:8080/svn/lqn/trunk-V6/libmva/regression/test4.cc $
 *
 ...
 *
 * ------------------------------------------------------------------------
 * $Id: test4.cc 17211 2024-05-13 22:13:11Z greg $
 * ------------------------------------------------------------------------
 */
...
#define case5
	...
	break;
    default:
	std::cerr << "Invalid V5 index (0-4):" << v5_ix << std::endl;
	exit( 1 );
    }
...
check( const int solverId, const MVA & solver, const unsigned v5_ix )
{
	...
	for ( unsigned k = 1; k <= solver.K; ++k ) {
		...
		std::cerr << ".  Computed=" << solver.L[n][m][1][k] << ", Correct= " << goodL[v5_ix][solverId][m][k] << std::endl;
		ok = false;
	    }
	}
    }
