/*  -*- c++ -*-
 * $HeadURL: http://rads-svn.sce.carleton.ca:8080/svn/lqn/branches/merge-V5-V6/libmva/regression/test3c.cc $
 *
 * Pg 264, Lazowska test.
 * FCFS with class dependent average service times.
 * Each Disk gets its own class.
 *
 *
 * ------------------------------------------------------------------------
 * $Id: test3c.cc 15385 2022-01-25 11:45:59Z greg $
 * ------------------------------------------------------------------------
 */
...
test( Population& NCust, Vector<Server *>& Q, VectorMath<double>& Z, VectorMath<unsigned>& priority, const unsigned s5k_ix )
{
    ...
    for ( k = 1; k <= 4; ++k ) {
	Q[5]->setService(k,service).setVisits(k,8.0);
    }

    Q[5]->setService(5,2.0).setVisits(5,20.0);
