...
man()
{
	...
	case 'I':
	    cout << "The " << current_option(i) << " is used to force the input file format to either \\fIxml\\fR, \\fIlqn\\fR, of \\fIjson\\fR." << endl
		 << "By default, if the suffix of the input filename is one of: \\fI.in\\fR, \\fI.lqn\\fR, \\fI.xlqn\\fR, of \\fI.txt\\fR," << endl
		 << "then the LQN parser will be used.  If the suffix of the input filename is one of: \\fI.json\\fR, \\fI.lqnj\\fR, \\fI.jlqn\\fR, of \\fI.lqjo\\fR," << endl
		 << "then the JSON parser will be used.  Otherwise, input is assumed to be XML." << endl;
	    break;
...
#endif
	    cout << ".TP" << endl
		 << "\\fB" << current_arg(i,FORMAT_JSON) << "\\fR" << endl
		 << "Generate an JSON input file.  If results are available, they are included." << endl
