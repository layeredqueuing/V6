/*  -*- c++ -*-
 * $Id: call.cc 16888 2023-12-08 12:18:20Z greg $
 *
...
Call::moveDst( const Point& aPoint )
{
    ...
    const double delta_y = Flags::y_spacing() / 3.0;
...
{
    return ( dstEntity()->isSelected() || Flags::include_only() )
	&& ( dynamic_cast<const Processor *>(dstEntity())->isInteresting()
	     || (Flags::chain() != 0 && dstEntity()->isSelectedIndirectly())
	     || submodel_output()
	     || queueing_output() );
}
...
ProcessorCall::updateRateFrom( const Call& call )
{
	...
	dstEntity()->getDOM()->runtime_error( LQIO::ERR_OPEN_AND_CLOSED_CLASSES );
	return *this;
    }
...
static std::ostream&
qnap_visits_str( std::ostream& output, const EntityCall& aCall )
{
