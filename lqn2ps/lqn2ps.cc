/*  -*- c++ -*-
 * $Id: lqn2ps.cc 15144 2021-12-02 19:10:29Z greg $
 *
...
lqn2ps( int argc, char *argv[] )
{
    ...
    sscanf( "$Date: 2021-12-02 14:10:29 -0500 (Thu, 02 Dec 2021) $", "%*s %s %*s", copyrightDate );
	...
	case 'L':
		...
		Flags::print[LAYERING].opts.value.i = arg;
...
//	    pragma( "tasks-only", "" );
	...
	case 512+'w':
	    Flags::print[PROCESSORS].opts.value.i = PROCESSOR_ALL;
	    Flags::print[LAYERING].opts.value.i = LAYERING_SRVN;
	    break;
...
//	Flags::print[PROCESSORS].opts.value.i = PROCESSOR_ALL;
    ...
    if ( submodel_output() && Flags::print[LAYERING].opts.value.i == LAYERING_SQUASHED ) {
    ...
    if ( Flags::print[PROCESSORS].opts.value.i == PROCESSOR_NONE 
	 || Flags::print[LAYERING].opts.value.i == LAYERING_PROCESSOR
	 || Flags::print[LAYERING].opts.value.i == LAYERING_SHARE ) {
    ...
    switch( Flags::print[OUTPUT_FORMAT].opts.value.i ) {
...
process( const std::string& input_file_name, const std::string& output_file_name, int model_no )
{
    ...
    switch ( Flags::print[INPUT_FORMAT].opts.value.o ) {
    ...
    LQIO::DOM::Document* document = LQIO::DOM::Document::load( input_file_name, input_format, errorCode, parse_file_name == 0 && Flags::print[RESULTS].opts.value.b );
    if ( !document ) {
	std::cerr << LQIO::io_vars.lq_toolname << ": Input model was not loaded successfully." << std::endl;
	LQIO::io_vars.error_count += 1;
	return;
    }
    if ( parse_file_name && Flags::print[RESULTS].opts.value.b ) {
	try {
...
//	pragmas.insert(LQIO::DOM::Pragma::_bcmp_,LQIO::DOM::Pragma::_true_);
    ...
    if ( !queueing_output()
	 && (   Flags::print[OUTPUT_FORMAT].opts.value.o == file_format::JMVA
	     || Flags::print[OUTPUT_FORMAT].opts.value.o == file_format::QNAP2) ) {
	std::cerr << LQIO::io_vars.lq_toolname << ": -O" << Options::io.at(Flags::print[OUTPUT_FORMAT].opts.value.o)
...
setOutputFormat( const file_format f ) 
{
    ...
    case file_format::LQX:
    case file_format::XML:
	Flags::print[LAYERING].opts.value.i = LAYERING_PROCESSOR;	/* Order by processors */
