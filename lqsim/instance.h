...
class srn_timeout_queue : public Virtual_Instance
{
    /* TIMEOUT_QUEUE 	*/
public:
    srn_timeout_queue( Task * cp, const char * task_name )
	: Virtual_Instance( cp, task_name ) {}

    virtual const char * type_name() const { return Task::type_strings[Task::TIMEOUT_QUEUE]; }
    void run();
    void timeout_ed( Message * msg, long entry_id, int * n_workers);

};

class srn_timeout_worker : public Virtual_Instance
{
    /* WORKER 		*/
public:
    ...
    virtual const char * type_name() const { return Task::type_strings[Task::TIMEOUT_WORKER]; }
    void run();
    void server_cycle_timeout(  Entry *, Message * msg, bool reschedule );
    void do_forwarding1 ( Message * msg, const Entry * ep ,bool doforwarding);
};


class srn_retry_queue : public Virtual_Instance
{
    /* RETRY_QUEUE 	*/
public:
    srn_retry_queue( Task * cp, const char * task_name )
	: Virtual_Instance( cp, task_name ) {}

    virtual const char * type_name() const { return Task::type_strings[Task::RETRY_QUEUE]; }
    void run();

    void retry_ed( Message * msg, long entry_id, int * n_workers);
    bool isInfRetry() {return _maxRetries <0;}
    void setRetries( int m_retries) {_maxRetries = m_retries;}
    int  getRetries()  {return _maxRetries;}
    void go_abort ( Message * msg, long entry_id,  bool doforwarding);

private:
    int _maxRetries;
};

class srn_retry_worker : public Virtual_Instance
{
    /* RETRY_WORKER 		*/
public:
    ...
    virtual const char * type_name() const { return Task::type_strings[Task::RETRY_WORKER]; }
    void run();
