 ...
 *
 * $Id: instance.cc 14609 2021-04-18 14:09:42Z greg $
 */
...
srn_worker::run()
{
	...
	if ( ps_send( _cp->worker_port(), -Task::WORKER, (char *)0, std_port() ) != OK ) {
	    abort();
	}
...
srn_token::run()
{
	...
	if ( ps_send( cp->worker_port(), -Task::WORKER, (char *)0, std_port() ) != OK ) {
	    abort();
	}
	...
	if ( ps_send( cp->signal_port(), -Task::WORKER, (char *)0, std_port() ) != OK ) {
	    abort();
	}
...
srn_writer_token::run()
{
	...
	if ( ps_send( cp->signal_port2(), -Task::WRITER_TOKEN, (char *)0, std_port() ) != OK ) {
	    abort();
	}
...
srn_thread::run()
{
	...
	if ( ps_send( pp->thread_port(), -Task::THREAD, (char *)ap, std_port() ) != OK ) {
	    abort();
	}
...
/*
 * Queue-type task has a timeout constrain. 
 * A dispatcher to inifinite number of workers doing abort ops.  
 * A dispatcher to a multiserver or a server task.
 */

void
srn_timeout_queue::run()
{
    timeline_trace( TASK_CREATED );
    Timeout_Task * cp = dynamic_cast<Timeout_Task *>(_cp);

    /* Create tokens */

    /* pre-created only 1 worker for abort */   

    Instance * task;
    task = new srn_timeout_worker( cp, name() );
    ps_resume( task->task_id() );
	...
	double server_time;
	double diff;
	double timeout = cp->_timeout;  //timeout pattern
	...
	if (cp->_timeout==0.0){
	    ...
	    rc = ps_receive( cp->ready_port() , IMMEDIATE, &server_id, &server_time,(char **)&worker_msg, &server_port ) ;
	    if ( rc == SYSERR ) {
	        abort();
	    } else if ( rc > 0 ) {
		...
		continue;
	        ...
	        timeout_ed( msg, entry_id, &n_workers);
	        continue;
	    }
	}
	...
	/*  timeout pattern*/
	if (diff >= timeout){
	  // timeout occurs, msg is dispatch to a timeout worker for cleanups.
	  timeout_ed( msg, entry_id, &n_workers);
	  continue;
	}
	...
	if ( rc == SYSERR ) {
	  ...
	  timeline_trace( WORKER_DISPATCH ); // msg is passing to server...
	  ...
	  continue;
	     ...
	     rc = ps_receive( cp->ready_port() , timeout-diff, &server_id, &server_time,(char **)&worker_msg, &server_port  );
	     if (rc == SYSERR ) {
	          abort();
	     } else if (rc>0){

	       /* server is avaliable before timeout. */
	         timeline_trace( WORKER_DISPATCH ); 
...
/*
 * Timeout Worker -- a non-reference task.  This type of task is an instance of
 * a multi-server.  They are dispatched from srn_timeout_queue tasks.
 */

void
srn_timeout_worker::run()
{
    timeline_trace( TASK_CREATED );
	...
	if ( ps_send( _cp->worker_port(), -Task::TIMEOUT_WORKER, (char *)0, ps_my_std_port ) != OK ) {
	    abort();
	}
	...
	timeline_trace( TASK_IS_RUNNING, 1 );
	...
	server_cycle_timeout( Entry::entry_table[entry_id], msg, start_time == ps_now );
...
/*
 *  The main server cycle of srn_timeout_worker task.
 */

void
srn_timeout_worker::server_cycle_timeout ( Entry * ep, Message * msg, bool reschedule )
{
   // double start_time 	= ps_my_schedule_time;
     double delta;
     Timeout_Task * cp= dynamic_cast<Timeout_Task *>(_cp);	
	...
	//ps_record_stat( cp->r_timeout_sqr.raw, square( delta ) );
     ...
     assert( ep->index() < _cp->n_entries() );

    /*
     * Start normal processing.
     */
     //const Entry * reply_ep=msg->client;
		
	do_forwarding1( msg, ep ,1);
	// ps_record_stat( _cp->r_cycle.raw, delta );		/* Task cycle time.	*/
...
/*
 * A request goes to timeout path.  
 * The srn_retry_queue is dispatching the request to one of timeout_workers.
 */

void
srn_timeout_worker::do_forwarding1 ( Message * msg, const Entry * ep ,bool doforwarding)
{
    if ( !msg ) {
	return;					/* No operation.	*/
	...
	const Targets& fwd = ep->get_fwd();	/* forwarding pointer	*/
...
/*
 * A request goes to timeout path.  
 * The srn_timeout_queue is dispatching the request to one of timeout workers.
 */

void
srn_timeout_queue::timeout_ed( Message * msg, long entry_id, int * n_workers)
{

    Timeout_Task * cp = dynamic_cast<Timeout_Task *>(_cp);

    /* Stuff for server (srn_timeout_worker) */
    ...
    if (rc == SYSERR ) {
	  ...
	  ps_send( worker_port, entry_id, (char *)msg, msg->reply_port );
           ...
           // no free worker available, create a new worker;
           Instance * task = new srn_timeout_worker( _cp, _cp->name() );
	   ps_resume( task->task_id() );
	   n_workers += 1;

	    /* 
	       ps_receive( cp->worker_port(), IMMEDIATE , &worker_id, &worker_time,
			       (char **)&worker_msg, &worker_port );
	       may have problem in receiving the ready message for the newly created worker.
	       so, using 'NEVER', and check the time period used for receiving.
	     */

	    double current_time = ps_now;
	    rc = ps_receive( cp->worker_port(), NEVER, &worker_id, &worker_time,
			       (char **)&worker_msg, &worker_port );
	    
	    if (rc == SYSERR ) {
	        abort();

	    } else if (rc > 0  && (ps_now - current_time ) <= 0.){
	     
	         timeline_trace( WORKER_DISPATCH );
		 //need to check whether keep the time stamp
		 //ps_resend( worker_port, entry_id, msg->time_stamp, (char *)msg, msg->reply_port ); 
		 ps_send( worker_port, entry_id, (char *)msg, msg->reply_port );
...
/*======================
 * Queue-type task has a retry constrain. 
 * A dispatcher to inifinite number of workers doing abort ops.  
 * A dispatcher to a multiserver or a server task.
 *
 *   nRetries is the mean number of retries;  N_req = r_nretry.mean_count();
 *   Yretry = N_retried /(N_req + N_retried);  N_total_request = r_Yretry.mean_count();
 *   r_abort_prob = N_abort / N_req; N_req = r_abort_prob.mean_count();
 */
...
{
    timeline_trace( TASK_CREATED );
    Retry_Task * cp = dynamic_cast<Retry_Task *>(_cp);

    /* Create tokens */

    // pre-created 1 worker for abort    
    Instance * task;
    task = new srn_retry_worker( cp, name() );
    ps_resume( task->task_id() );
    int  n_workers = 1;
	...
	int rc;
	Message * worker_msg;
	double nretries;           /* number of retries*/
...
/*
 * A request goes to retry path.  
 * The srn_retry_queue is dispatching the request to one of retry_workers.
 */

void
srn_retry_queue::retry_ed( Message * msg, long entry_id, int * n_workers)
{

	 Retry_Task * cp = dynamic_cast<Retry_Task *>(_cp);

	/* Stuff for server (srn_retry_worker) */
	...
	if (rc == SYSERR ) {
	    ...
	    // no free worker available, create a new worker;
	    Instance * task = new srn_retry_worker( cp, cp->name() );
	    ps_resume( task->task_id() );
	    n_workers += 1;
...
/*
 * Retry Worker -- a non-reference task.  This type of task is an instance of
 * a multi-server.  They are dispatched from srn_retry_queue tasks.
 */


void
srn_retry_worker::run()
{
    timeline_trace( TASK_CREATED );
	...
	if ( ps_send( _cp->worker_port(), -Task::RETRY_WORKER, (char *)0, ps_my_std_port ) != OK ) {
	    abort();
	}
	...
	server_cycle_retry( entry_id, msg, start_time == ps_now );
...
/*
 * Main server cycle of retry worker task.  
 * The difference between this worker with regular worker tasks are:
 *    1. keep the initial time stamp;
 *    2. send request back to the std_port of the retry server queue;
 *    3. do nothing and sleep;
 * note: using this worker; there is no need to specify a task in the input file;
 */

void
srn_retry_worker::server_cycle_retry ( long entry_id , Message * msg, bool reschedule )
{
	...
	//keep the time stamp
	ps_resend( cp->std_port(), entry_id,  msg->time_stamp, (char *)msg, msg->reply_port );	

	ps_my_schedule_time = ps_now;		/* In case we don't block...	*/
}

void
srn_retry_queue::go_abort ( Message * msg, long entry_id,  bool doforwarding)
{
...
Instance::do_forwarding ( Message * msg, const Entry * ep )
{
	...
	if ( !tp ) {
	    ...
	    /* Reply to sender	*/
		//if (_cp->type()==Task::TIMEOUT_QUEUE)  //printf("reply\n");
	    long reply_port  = msg->reply_port;	/* Local copy	*/
...
Instance::timeline_trace( const trace_events event, ... )
{
	...
	case QUEUE_TIMEOUT:
	    (void) fprintf( stddbg, "Queue Timeout." );
	    break;

	case MSG_RETRY:
	    (void) fprintf( stddbg, "Request Retried." );
	    break;

	case MSG_ABORT:
	    (void) fprintf( stddbg, "Request Aborted." );
	    break;
