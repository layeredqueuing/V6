/*	-*- c++ -*-
 * $Id: confidence_intervals.cpp 13015 2017-05-10 20:16:53Z greg $
 *
...
namespace LQIO {
    ...
    /* static */ double 
    ConfidenceIntervals::invert( const double interval, const unsigned blocks, const confidence_level_t level )
    {
	if ( interval == 0. ) return 0.0;
	const double stddev = interval / get_t_value( blocks, level );
	return stddev * stddev;
    }


    ConfidenceIntervals::ConfidenceIntervals( const confidence_level_t level, const unsigned blocks )
	: _t_value(0), _level(level), _blocks(blocks)
    {
	if ( blocks > 1 ) {
	    _t_value = get_t_value( blocks, level );
	}
    }
    ...
    ConfidenceIntervals& 
    ConfidenceIntervals::set_level( const confidence_level_t level )
    {
	_level = level;
	_t_value = get_t_value( _blocks, level );
	assert( _t_value != 0.0 );
	return *this;
    }

    double 
    ConfidenceIntervals::operator()( const double variance ) const
    {
	return sqrt( variance ) * _t_value;
    }

    double
    ConfidenceIntervals::invert( const double interval ) const
    {
	const double stddev = interval / _t_value;
	return stddev * stddev;
    }
};
