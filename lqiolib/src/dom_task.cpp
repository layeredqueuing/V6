/*
 *  $Id: dom_task.cpp 14215 2020-12-14 19:09:04Z greg $
 *
...

	...
	TimeoutTask::TimeoutTask(const Document * document, const char * name,
				 const std::vector<DOM::Entry *>& entryList,
				 const Processor* processor,
				 ExternalVariable* queue_length, ExternalVariable * priority,
				 ExternalVariable* n_copies, ExternalVariable* n_replicas,
				 const Group * group)
	...
	void TimeoutTask::setDecision(Decision * aDecision)
	{
	    ...
	    /* update variables */
	    setTimeoutValue( aDecision->getTimeoutValue() );
	    setAbortValue( aDecision->getAbortValue() );

	}

	void TimeoutTask::setAbort( ExternalVariable *  abort)
	{
	    _abort = abort;
	}
	...
	void TimeoutTask::setTimeout( ExternalVariable *  timeout )
	{
	    _timeout = timeout;
	}
	...
	ExternalVariable *  TimeoutTask::getAbort() const
	{
	    return _abort;
	}

	double TimeoutTask::getAbortValue() const
	{
	    /* Retun the retry sleep time */
	    double value = 0.0;
	...
	ExternalVariable *  TimeoutTask::getTimeout() const
	{
	    return _timeout;
	}

	double TimeoutTask::getTimeoutValue() const
	{
	    /* Retun the retry sleep time */
	    double value = 0.0;
	...
	bool TimeoutTask::hasHistogram() const
	{
	    return _histogram != 0 && _histogram->getBins() > 0;
	}

	void TimeoutTask::setHistogram(Histogram* histogram)
	{
	    /* Stores the given Histogram of the Phase */
	    _histogram = histogram;
	}
	...
	RetryTask::RetryTask(const Document * document, const char * name,
			     const std::vector<DOM::Entry *>& entryList,
			     const Processor* processor,
			     ExternalVariable* queue_length, ExternalVariable * priority,
			     ExternalVariable* n_copies, ExternalVariable* n_replicas,
	...
	RetryTask::RetryTask(const Document * document, const char * name,
	    ...
	    : Task(document, name, SCHEDULE_RETRY, entryList, processor,
		   queue_length, priority, n_copies, n_replicas, group ),
	      _sleep(sleeptime), _maxRetries(maxreties), _abort(abort),
	      _resultSuccessProbability(0.), _resultSuccessProbabilityVariance(0.),
	      _resultAbortProbability(0.), _resultAbortProbabilityVariance(0.),
	      _resultNumberOfRetries(0.), _resultNumberOfRetriesVariance(0.),
	      _decision(nullptr), _histogram(nullptr)
	...
	void RetryTask::setDecision(Decision * aDecision)
	{
	    ...
	    /* update variables */
	    setSleepTimeValue( aDecision->getSleepValue() );
	    setMaxRetriesValue( aDecision->getMaxRetryValue() );
	    setAbortValue( aDecision->getAbortValue() );

	}

	void RetryTask::setSleepTime( ExternalVariable* sleeptime)
	{
	    _sleep = sleeptime;
	}
	...
	void RetryTask::setMaxRetries( ExternalVariable* maxretries )
	{
	...
	ExternalVariable* RetryTask::getSleepTime() const
	{
	    return _sleep;
	}

	double RetryTask::getSleepTimeValue() const
	{
	    /* Retun the retry sleep time */
	    double value = 0.0;
	...
	ExternalVariable* RetryTask::getMaxRetries() const
	{
	    return _maxRetries;
	}

	int RetryTask::getMaxRetriesValue() const
	{
	    /* Retun the number of maximum retries */
	    double value = 0.0;
	    assert( _maxRetries->getValue(value) == true);
	    return static_cast<int>(value);
	}

	void RetryTask::setAbort( ExternalVariable* abort)
	{
	    _abort = abort;
	}
	...
	ExternalVariable* RetryTask::getAbort() const
	{
	    return _abort;
	}

	double RetryTask::getAbortValue() const
	{
	    /* Retun the abort time */
	    double value = 0.0;
	    assert(_abort->getValue(value) == true);
	    assert(value >= 0);
	    return static_cast<double>(value);
