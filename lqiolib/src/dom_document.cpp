/*
 *  $Id: dom_document.cpp 15311 2021-12-31 22:23:29Z greg $
 *
...
namespace LQIO {
	...
	{
	    /* Store the group in the map regarless of existence */
	    _decisions[decision->getName()] = decision;
	}
 
	Decision* Document::getDecisionByName(const std::string& name) const
	{
	    std::map<std::string,Decision*>::const_iterator decision = _decisions.find(name);
	    /* Attempt to find the group with the given name */
	...
	DecisionPath* Document::getDecisionPathByName(const std::string& name) const
	{
	    std::map<std::string,DecisionPath*>::const_iterator decisionPath = _decisionPaths.find(name);
	    /* Attempt to find the group with the given name */
	...
	const std::map<std::string,DecisionPath*>& Document::getDecisionPaths() const
	{
	    /* Return the pointer */
	    return _decisionPaths;
	}
...

	...
	Document::print( const std::string& output_file_name, const std::string& suffix, OutputFormat output_format, bool rtf_output ) const
	{
		...
		output_format = input_to_output_format.at( getInputFormat() );
	    }
	...
	Document::print( const std::string& output_file_name, const std::string& suffix, OutputFormat output_format, bool rtf_output, unsigned int iteration ) const
	{
	    ...
	    if ( ext_iter != __output_extensions.end() ) {
	    ...
	    LQIO::Filename filename( __input_file_name, extension, directory_name, lqx_output ? suffix : std::string("") );
