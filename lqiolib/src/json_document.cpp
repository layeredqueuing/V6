/* -*- c++ -*-
 * $Id: json_document.cpp 17458 2024-11-12 11:54:17Z greg $
 *
 ...
 
	...
	JSON_Document::handleProcessor( DocumentObject *, const picojson::value& value )
	{
		...
		try {
			...
			for (picojson::value::object::const_iterator i = obj.begin(); i != obj.end(); ++i) {
			    ...
			    } else if ( createObjects() || j->second.getType() == dom_type::JSON_OBJECT ) {	/* Bug 469 */
	...
	JSON_Document::handleActivity( DocumentObject * parent, const picojson::value& value )
	{
		...
		try {
		    ...
		    Activity * activity = task.getActivity( activity_name, createObjects() );
		    if ( createObjects() ) {
	...
	JSON_Document::handlePrecedence( DocumentObject * parent, const picojson::value& value )
	{
		...
		if ( !deferred.empty() && dynamic_cast<AndJoinActivityList *>(pre_list) == nullptr ) {
			...
			if ( i->first == Xquorum && i->second.is<double>() && createObjects() ) {
			    dynamic_cast<AndJoinActivityList *>(pre_list)->setQuorumCountValue( static_cast<int>(i->second.get<double>()) );
			} else if ( i->first == Xquorum && i->second.is<std::string>() && createObjects() ) {
	...
	JSON_Document::handleCall( DocumentObject * parent, const picojson::value& value, Call::Type call_type )
	{
		...
		try {
			...
			if ( !call &&  createObjects() ) {		/* Skip nulls as we don't want to create things */
...

	...
	JSON_Document::ExportComment::print( const Document& document ) const
	{
	    ...
	    _output << indent() << "\"" << tbuf << "\",";
#endif
	    _output << indent() << "\"Invoked as: " << LQIO::io_vars.lq_command_line << ' ' << LQIO::DOM::Document::__input_file_name.string() << "\",";
