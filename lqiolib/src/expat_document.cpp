...
/* !!! SPEX document observations */
        ...
        */
	    ...
	    } else if ( strcasecmp( element, Xdecision) == 0 ) {
                Decision * decision = handleDecision( task, attributes );
                _stack.push( parse_stack_t(element,&Expat_Document::startDecisionType,decision) );
...
/*
  ...
  <xsd:annotation>
  <xsd:documentation>Decisions has own processor and task.</xsd:documentation>
  </xsd:annotation>
  <xsd:sequence>
  <xsd:element name="result-decision" type="OutputResultType" minOccurs="0" maxOccurs="unbounded"/>
  <xsd:element name="path" type="DecisionPathType"  minOccurs="2" maxOccurs="3"/>
  <xsd:element name="processor" type="ProcessorType" minOccurs="0" maxOccurs="1"/>
  </xsd:sequence>
  </xsd:complexType>
*/

        void
        Expat_Document::startDecisionType( DocumentObject * decision, const XML_Char * element, const XML_Char ** attributes )
        {

            if ( strcasecmp( element, Xresult_decision) == 0 ) {
		try {
		...
		try {
            ...
            } else if ( strcasecmp( element, Xpath) == 0 ) {
                DecisionPath * path = handleDecisionPath( decision, attributes );
                _stack.push( parse_stack_t(element,&Expat_Document::startDecisionPathType,path) );

		/*  } else if ( strcasecmp( element, Xprocessor) == 0 ) {
		    Processor * processor = handleProcessor( decision, attributes );
		    _stack.push( parse_stack_t(element,&Expat_Document::startProcessorType,processor) );
		*/
            } else {
                throw element_error( element );
...
/*
  <xsd:complexType name="DecisionPathType">
  <xsd:annotation>
  <xsd:documentation>Decision path has attributes and results.</xsd:documentation>
  </xsd:annotation>
  <xsd:sequence>
  <xsd:element name="result-decision-path" type="OutputResultType" minOccurs="0" maxOccurs="unbounded"/>
  </xsd:sequence>
  </xsd:complexType>
*/

        void
        Expat_Document::startDecisionPathType( DocumentObject * decisionpath, const XML_Char * element, const XML_Char ** attributes )
        {

            if ( strcasecmp( element, Xresult_decision_path) == 0 ) {
		try {
...

        ...
        */
		...
		} else if ( sched_type == SCHEDULE_TIMEOUT || sched_type == SCHEDULE_ABORT) {
		    task = new TimeoutTask( &_document, task_name, entries, processor,
					    getOptionalAttribute(attributes,Xqueue_length),
					    getOptionalAttribute(attributes,Xpriority),
					    getOptionalAttribute(attributes,Xmultiplicity),
					    getOptionalAttribute(attributes,Xreplication),
		...
		} else if ( sched_type == SCHEDULE_RETRY || sched_type == SCHEDULE_INF_RETRY) {
                    task = new RetryTask( &_document, task_name, entries, processor,
					  getOptionalAttribute(attributes,Xqueue_length),
					  getOptionalAttribute(attributes,Xpriority),
					  getOptionalAttribute(attributes,Xmultiplicity),
					  getOptionalAttribute(attributes,Xreplication),
	...
	Expat_Document::handleDecision( DocumentObject * task, const XML_Char ** attributes )
	{
	    ...
	    const XML_Char * decision_name = getStringAttribute(attributes,Xname);
	    const XML_Char * decision_entry_name = getStringAttribute(attributes,Xdecision_entry);

	    Decision* decision = _document.getDecisionByName( decision_name );

	    if ( _createObjects ) {
		if ( decision ) {
		    throw duplicate_symbol( decision_name );
		} else {
		    /*const scheduling_type decision_type = getSchedulingAttribute( attributes, Xdecision_type );
		      if (! ( (decision_type == SCHEDULE_TIMEOUT) ||
		    ...
		    */
		    ...
		    if ( strcasecmp( type, Xtimeout ) == 0 ) {
			decision_type = TYPE_TIMEOUT;
		    } else if ( strcasecmp( type, Xabort ) == 0 ) {
			decision_type = TYPE_ABORT;
		    } else if ( strcasecmp( type, Xretry ) == 0 ) {
			decision_type = TYPE_RETRY;
		    ...
		    decision  = new Decision( &_document, dynamic_cast<Task *>(task), 
			decision_name, decision_type,  decision_entry_name,
			_document.db_build_parameter_variable(getStringAttribute(attributes,Xtimeout,"0.0"),NULL),
			_document.db_build_parameter_variable(getStringAttribute(attributes,Xabort,"0.0"),NULL),
			_document.db_build_parameter_variable(getStringAttribute(attributes,Xsleep,"0.0"),NULL),
			_document.db_build_parameter_variable(getStringAttribute(attributes,Xmax_retries,"0"),NULL)
			);

		    _document.addDecision(decision);
		    //decision->printDecision();
		    if (decision_type == TYPE_RETRY  || decision_type == TYPE_INF_RETRY) {
			dynamic_cast<RetryTask *>(task)->setDecision( decision);
		    } else {
		        dynamic_cast<TimeoutTask *>(task)->setDecision( decision);
		    }
		}
            } else if ( !decision ) {
                throw undefined_symbol( decision_name );
            }
            return decision;
	}
	...
	/*
          ...
          <xsd:attribute name="type" type="DecisionPathType" use="required"/>
          <xsd:attribute name="name" type="xsd:string" use="required"/>
        */
	DecisionPath *
	Expat_Document::handleDecisionPath( DocumentObject * decision, const XML_Char ** attributes )
	{
	    ...
	    const XML_Char * decision_path_name = getStringAttribute(attributes,Xname);
	    const XML_Char * toEntry_name = getStringAttribute(attributes,Xto_entry);
            DecisionPath * decisionPath = _document.getDecisionPathByName( getStringAttribute(attributes,Xname) );
            if ( _createObjects ) {
                if ( decisionPath ) {
                    throw duplicate_symbol( decision_path_name );
                } else{

		    const XML_Char * pathType = getStringAttribute(attributes,Xtype,"");
		    if ( strcasecmp( pathType, Xsucc ) == 0 ) {
			decisionPath = new DecisionPath(&_document, decision_path_name, dynamic_cast<Decision *>(decision), SUCCESS_PATH,  toEntry_name);

		    } else if ( strcasecmp( pathType, Xabort ) == 0 ) {
			decisionPath = new DecisionPath(&_document, decision_path_name, dynamic_cast<Decision *>(decision), ABORT_PATH,  toEntry_name);

		    } else if ( strcasecmp( pathType, Xtimeout ) == 0 ) {
			decisionPath = new DecisionPath(&_document, decision_path_name, dynamic_cast<Decision *>(decision), TIMEOUT_PATH,  toEntry_name);

		    } else if ( strcasecmp( pathType, Xretry ) == 0 ) {
			decisionPath = new DecisionPath(&_document, decision_path_name, dynamic_cast<Decision *>(decision), RETRY_PATH,  toEntry_name);

		    }
		    //if (decisionPath !=NULL)
		    //	decisionPath->printDecisionPath();
		}
	    }
	    return decisionPath;
	}
...

	...
	Expat_Document::exportSPEXParameters( std::ostream& output ) const
	{
        ...
        */
		...
		output << attribute( Xtimeout, dynamic_cast<const TimeoutTask&>(task).getTimeoutValue() );
		output << attribute( Xabort, dynamic_cast<const TimeoutTask&>(task).getAbortValue() );

            }else if ( task.getSchedulingType() == SCHEDULE_INF_RETRY || task.getSchedulingType() == SCHEDULE_RETRY  ) {
		output << attribute( Xsleep, dynamic_cast<const RetryTask&>(task).getSleepTimeValue() );
		output << attribute( Xabort, dynamic_cast<const RetryTask&>(task).getAbortValue() );
		output << attribute( Xmax_retries, dynamic_cast<const RetryTask&>(task).getMaxRetriesValue() );
            }
		...
		if ( _document.hasBottleneckStrength() ) {
		    output << attribute( Xbottleneck_strength, task.getResultBottleneckStrength() );
                ...
                } else if ( dynamic_cast<const TimeoutTask *>(&task) ) {
                    output << attribute( Xtimeout_prob, dynamic_cast<const TimeoutTask *>(&task)->getResultTimeoutProbability() )
                           << attribute( Xtimeout_prob_variance, dynamic_cast<const TimeoutTask *>(&task)->getResultTimeoutProbabilityVariance() )
                           << attribute( Xsuccess_prob, dynamic_cast<const TimeoutTask *>(&task)->getResultSuccessProbability() );

		} else if ( dynamic_cast<const RetryTask *>(&task) ) {
		    output << attribute( Xabort_prob, dynamic_cast<const RetryTask *>(&task)->getResultAbortProbability() )
			   << attribute( Xabort_prob_variance, dynamic_cast<const RetryTask *>(&task)->getResultAbortProbabilityVariance() )
			   << attribute( Xsuccess_prob, dynamic_cast<const RetryTask *>(&task)->getResultSuccessProbability() )
			   << attribute( Xnumber_of_retries, dynamic_cast<const RetryTask *>(&task)->getResultNumberOfRetries() )
			   << attribute( Xnumber_of_retries_variance, dynamic_cast<const RetryTask *>(&task)->getResultNumberOfRetriesVariance() );
                }
...

	    ...
	    Common_IO::init_tables();
            ...
            decision_table.insert(Xname);
	    decision_table.insert(Xentry_name);
            decision_table.insert(Xtype);
	    decision_table.insert(Xfrom_entry);
            decision_table.insert(Xdecision_entry);
            decision_table.insert(Xtimeout);
            decision_table.insert(Xabort);
            decision_table.insert(Xsleep);
            decision_table.insert(Xmax_retries);

            decision_path_table.insert(Xname);
	    decision_path_table.insert(Xentry_name);
            decision_path_table.insert(Xtype);
            decision_path_table.insert(Xto_entry);
...
namespace LQIO {
	...
	const XML_Char * Expat_Document::Xspex_results =			"results-collection";
        ...
        const XML_Char * Expat_Document::Xsucc =                                "succ";
        const XML_Char * Expat_Document::Xretry =                               "retry";
	const XML_Char * Expat_Document::Xinf_retry =                           "inf-retry";
	const XML_Char * Expat_Document::Xdecision =                            "decision";
	const XML_Char * Expat_Document::Xresult_decision =                 	"result-decision";
	const XML_Char * Expat_Document::Xpath =                                "path";
	const XML_Char * Expat_Document::Xresult_decision_path =                "result-decision-path";
	const XML_Char * Expat_Document::Xentry_name =                          "entry-name";
	const XML_Char * Expat_Document::Xdecision_entry =                    	"decision-entry";
	const XML_Char * Expat_Document::Xfrom_entry =                        	"fromEntry";
	const XML_Char * Expat_Document::Xto_entry =                          	"toEntry";
	const XML_Char * Expat_Document::Xtimeout =                          	"timeout";
	const XML_Char * Expat_Document::Xabort =                            	"abort";
	const XML_Char * Expat_Document::Xmax_retries =                      	"max-retries";
	const XML_Char * Expat_Document::Xsleep =                            	"sleep";
	const XML_Char * Expat_Document::Xtimeout_prob  =                 	"timeout-prob";
	const XML_Char * Expat_Document::Xtimeout_prob_variance  =        	"timeout-prob-variance";
	const XML_Char * Expat_Document::Xsuccess_prob =                 	"success-prob";
	const XML_Char * Expat_Document::Xsuccess_prob_variance  =        	"success-prob-variance";
	const XML_Char * Expat_Document::Xabort_prob  =                   	"abort-prob";
	const XML_Char * Expat_Document::Xabort_prob_variance  =          	"abort-prob-variance";
	...
	std::map<const XML_Char, const XML_Char *> Expat_Document::escape_table;
        ...
        std::set<const XML_Char *,Expat_Document::attribute_table_t> Expat_Document::decision_table;
        std::set<const XML_Char *,Expat_Document::attribute_table_t> Expat_Document::decision_path_table;
