...
/*
 * $Id: symtbl.cc 14000 2020-10-25 12:50:53Z greg $
 *
...
static std::set<st_entry *,ltSymbol> symtbl[ST_SYMTBL_TYPES];	/* Primary table.	*/
static std::vector<st_entry *> postbl[ST_SYMTBL_TYPES];		/* Auxiliary index 	*/
...
find_symbol_name ( const char *name, const st_etypes type)
{
    ...
    std::set<st_entry *,ltSymbol>::const_iterator nextSymbol = find_if( symtbl[type].begin(), symtbl[type].end(), eqSymbol( name ) );
...
{
    for ( unsigned i = 0; i < ST_SYMTBL_TYPES; i++) {
	for ( std::set<st_entry *,ltSymbol>::const_iterator nextEntry = symtbl[i].begin(); nextEntry != symtbl[i].end(); ++nextEntry ) {
